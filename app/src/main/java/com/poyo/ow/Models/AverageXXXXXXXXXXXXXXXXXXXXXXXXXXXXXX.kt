package com.poyo.ow.Models

data class AverageXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX(
    val allDamageDoneAvgPer10Min: Float,
    val barrierDamageDoneAvgPer10Min: Float,
    val criticalHitsAvgPer10Min: Double,
    val deathsAvgPer10Min: Double,
    val eliminationsAvgPer10Min: Double,
    val eliminationsPerLife: Double,
    val finalBlowsAvgPer10Min: Double,
    val healingDoneAvgPer10Min: Float,
    val heroDamageDoneAvgPer10Min: Float,
    val meleeFinalBlowsAvgPer10Min: Double,
    val objectiveKillsAvgPer10Min: Double,
    val objectiveTimeAvgPer10Min: String,
    val soloKillsAvgPer10Min: Double,
    val timeSpentOnFireAvgPer10Min: String
)